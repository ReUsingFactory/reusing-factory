...
function submitOrder(){var sl=(cfg.hw==='cpu'?SCPU:SMB).find(function(s){return s.v===cfg.sock;}),tot=cT();var d=new FormData();d.append('form-name','order');d.append('hardware',cfg.hw);d.append('sockel',sl?sl.l:cfg.sock);if(cfg.hw==='cpu'){d.append('bent',cfg.bent||'');d.append('pins',cfg.pins||'');d.append('self',cfg.self||'');}if(cfg.hw==='mainboard')d.append('disasm',cfg.disasm||'');d.append('vip',cfg.vip||'');d.append('damage',cfg.dmg||'');d.append('country',cfg.country);d.append('name',cfg.cname||'');d.append('email',cfg.cemail||'');d.append('netto',fm(tot.n));d.append('mwst',fm(tot.m));d.append('brutto',fm(tot.b));fetch('/',{method:'POST',body:d}).catch(function(){});}
