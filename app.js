...
function submitOrder(){var sl=(cfg.hw==='cpu'?SCPU:SMB).find(function(s){return s.v===cfg.sock;}),tot=cT();var d={hardware:cfg.hw,sockel:sl?sl.l:cfg.sock,bent:cfg.bent||'',pins:cfg.pins||'',self_repair:cfg.self||'',disasm:cfg.disasm||'',vip:cfg.vip||'',damage:cfg.dmg||'',country:cfg.country,name:cfg.cname||'',email:cfg.cemail||'',netto:fm(tot.n),mwst:fm(tot.m),brutto:fm(tot.b),_subject:'Neuer Auftrag – ReUsing Factory',_template:'table',_captcha:'false'};fetch('https://formsubmit.co/ajax/kontakt@reusing-factory.de',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(d)}).catch(function(){});}
