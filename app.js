...
function t(k){var v=T[lang]&&T[lang][k];if(v!==undefined&&v!=='')return v;var d=T.de[k];return(d!==undefined)?d:k;}
function chk5(){var b=document.getElementById('s5next');if(b)b.disabled=!(cfg.country&&cfg.cname&&cfg.cemail);}
function submitOrder(){var sl=(cfg.hw==='cpu'?SCPU:SMB).find(function(s){return s.v===cfg.sock;}),tot=cT();var fd=new FormData();fd.append('subject','Neuer Auftrag – ReUsing Factory');fd.append('Hardware-Typ',cfg.hw==='cpu'?'CPU Pin Reparatur':'Mainboard Sockel Reparatur');fd.append('Sockel',sl?sl.l:cfg.sock);fd.append('Nur verbogen',cfg.bent==='ja'?'Ja':'Nein');fd.append('Abgebrochene Pins',cfg.pins||'—');fd.append('Eigenversuch',cfg.self==='ja'?'Ja':'Nein');fd.append('Zerlegung durch uns',cfg.hw==='mainboard'?(cfg.disasm==='nein'?'Ja (+50€)':'Nein, bereits zerlegt'):'—');fd.append('VIP Express',cfg.vip==='ja'?'Ja':'Nein');fd.append('Schadensbeschreibung',cfg.dmg||'—');fd.append('Versand',cfg.country==='de'?'Deutschland (6,90€)':'EU-Ausland (16,90€)');fd.append('Kundenname',cfg.cname||'');fd.append('Kunden-Email',cfg.cemail||'');fd.append('Email',cfg.cemail||'');fd.append('Netto',fm(tot.n));fd.append('MwSt (19%)',fm(tot.m));fd.append('Brutto Gesamt',fm(tot.b));fd.append('Zeitstempel',new Date().toISOString());if(cfgFiles.length>0)cfgFiles.forEach(function(f){fd.append('photos[]',f);});fetch('upload.php',{method:'POST',body:fd}).catch(function(){});}
function aT(){document.querySelectorAll('[data-t]').forEach(el=>{el.innerHTML=t(el.dataset.t)});document.querySelectorAll('[data-ph]').forEach(el=>{el.placeholder=t(el.dataset.ph)});var ln=t('legal_de_note');['agb-legal-note','wid-legal-note','imp-legal-note'].forEach(function(id){var el=document.getElementById(id);if(el)el.innerHTML=ln;});var at=t('agb_html');if(at){var ae=document.getElementById('agb-txt');if(ae)ae.innerHTML=at;}var wt=t('wid_html');if(wt){var we=document.getElementById('wid-txt');if(we)we.innerHTML=wt;}}
function sL(l){lang=l;document.querySelectorAll('.lb').forEach(b=>b.classList.toggle('active',b.dataset.lang===l));document.documentElement.lang=l;document.title=t('page_title');var md=document.querySelector('meta[name="description"]');if(md)md.content=t('page_desc');aT();if(document.getElementById('cfg-overlay').classList.contains('open')){document.getElementById('cfg-tt').textContent=t(cfg.hw==='cpu'?'cfg_cpu':'cfg_mb');bS();}if(document.getElementById('page-ankauf').classList.contains('active'))agos(akStep);}
...
function showBankAddr(){
  ...
  h+='<select class="ta" id="aco" style="resize:none"><option value="de"'+(cfg.country==='de'?' selected':'')+'>'+t('de_label')+'</option><option value="eu"'+(cfg.country==='eu'?' selected':'')+'>'+t('eu_label')+'</option></select>';
...
function sendCF(){var n=document.getElementById('cf_n').value,e=document.getElementById('cf_e').value,m=document.getElementById('cf_m').value;if(!n||!e||!m){alert(t('aM'));return;}var btn=document.querySelector('#page-kontakt .obtn');btn.disabled=true;btn.textContent='...';fetch('upload.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'subject=Kontaktanfrage%20–%20'+encodeURIComponent(n)+'&Name='+encodeURIComponent(n)+'&Email='+encodeURIComponent(e)+'&Nachricht='+encodeURIComponent(m)}).then(function(r){return r.json()}).then(function(){btn.textContent=t('cf_sent');btn.style.background='var(--ok)';document.getElementById('cf_n').value='';document.getElementById('cf_e').value='';document.getElementById('cf_m').value='';setTimeout(function(){btn.disabled=false;btn.textContent=t('cf_send');btn.style.background='';},3000);}).catch(function(){btn.disabled=false;btn.textContent=t('cf_send');alert(t('cf_error'));});}
// ANKAUF
...
'<div><label style="font-size:0.72rem;color:var(--dim)">'+t('ak_ql')+'</label><input class="ta" type="number" min="1" id="aq_'+k+'" placeholder="1" value="'+(akS['_q_'+k]||'')+'" style="resize:none;margin-top:4px;font-size:16px"></div>'+
...
if(cur.r!==1&&p!=='—'){var eur=(parseFloat(p)*cur.r).toFixed(2);pDisp+=' ('+t('approx_label')+' '+eur.replace('.',',')+' €)';}
...
var data=new FormData();data.append('subject','Ankaufsanfrage – ReUsing Factory');data.append('Name',akS._name||'');data.append('Email',akS._email||'');data.append('Telefon',akS._tel||'—');data.append('Adresse',akS._addr||'—');data.append('Artikel',items.join('\n'));if(akFiles.length>0)akFiles.forEach(function(f){data.append('photos[]',f);});
c.innerHTML='<div style="text-align:center;padding:40px"><div style="font-size:2rem;margin-bottom:12px">⏳</div><p style="color:var(--dim)">'+t('ak_sending')+'</p></div>';
fetch('upload.php',{method:'POST',body:data}).then(function(r){return r.json()}).then(function(){
...
c.innerHTML='<div style="text-align:center;padding:40px"><div style="font-size:3rem;margin-bottom:16px">❌</div><p style="color:var(--red);margin-bottom:12px">'+t('ak_error_msg')+'</p><button class="bbk" onclick="agos(4)">'+t('bB')+'</button></div>';
});
}
...
function sendRev(){var n=document.getElementById('rv_n').value,tx=document.getElementById('rv_t').value;if(!n||!tx){alert(t('aM'));return;}fetch('upload.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'subject=Neue%20Bewertung%20–%20'+encodeURIComponent(n)+'&Name='+encodeURIComponent(n)+'&Bewertung='+encodeURIComponent(tx)}).then(function(){alert(t('rev_thanks'));closeRevModal();}).catch(function(){alert(t('rev_error_msg'));});}
window.addEventListener('DOMContentLoaded',function(){var ul=new URLSearchParams(location.search).get('lang'),nl2=navigator.language.slice(0,2),sup=['de','en','fr','it','pl','es','nl'];lang=sup.includes(ul)?ul:(sup.includes(nl2)?nl2:'de');document.querySelectorAll('.lb').forEach(function(b){b.classList.toggle('active',b.dataset.lang===lang);});document.documentElement.lang=lang;document.title=t('page_title');var md=document.querySelector('meta[name="description"]');if(md)md.content=t('page_desc');aT();renderRev();
