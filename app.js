...
function submitOrder(){var sl=(cfg.hw==='cpu'?SCPU:SMB).find(function(s){return s.v===cfg.sock;}),tot=cT();var d={access_key:'55063975-8c2f-4676-8af1-a36886eb2297',subject:'Neuer Auftrag – ReUsing Factory',from_name:'ReUsing Factory Website',hardware:cfg.hw,sockel:sl?sl.l:cfg.sock,bent:cfg.bent||'',pins:cfg.pins||'',self_repair:cfg.self||'',disasm:cfg.disasm||'',vip:cfg.vip||'',damage:cfg.dmg||'',country:cfg.country,name:cfg.cname||'',email:cfg.cemail||'',netto:fm(tot.n),mwst:fm(tot.m),brutto:fm(tot.b)};fetch('https://api.web3forms.com/submit',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(d)}).catch(function(){});}
